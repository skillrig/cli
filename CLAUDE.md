...
The test tiers map to the package layout (constitution §III), three of them: (1) presentation-free **unit** tests in `internal/...` + `pkg/...`; (2) the **integration/quickstart** acceptance suite in `test/` (package `quickstart`) — `TestQuickstart_*` functions that build the real binary and exec it against a *stubbed* substrate (a `file://` origin or a fake `git`), each mapping 1:1 to a `quickstart.md` scenario; (3) the distinct true-substrate **e2e** tier in `test/e2e/` (build tag `e2e`), whose tests use the **`TestE2E_*`** prefix — deliberately NOT `TestQuickstart_*`, because they are not quickstart-contract scenarios (see the loop-closing note below). Run a single test directly, e.g. `go test ./internal/config -run TestParseOrigin`. `mise.toml` only provisions the `crit` review tool, not build tooling.
...
- `go test`, three tiers — (a) presentation-free **unit** in `internal/...` + `pkg/skillcore` (table-driven + ground-truth: fetched tree-SHA == raw `git`; `index` output == committed `index.json`); (b) **`TestQuickstart_*` integration** in `test/` building/exec'ing the real binary against a stubbed substrate; (c) **`TestE2E_*` true-substrate e2e** in `test/e2e/` (build tag `e2e`, `make test-e2e`, excluded from CI) — a real `git http-backend` + auth gate proving credential authentication. **Integration network boundary** tested via S4's substrate: `file://` + local bare repo for happy/integrity; the `pkg/skillcore/git.go` `commandContext` exec-stub seam (extended to `Clone`/`FetchSparse`) for auth/unreachable/transient. **No `httptest`/go-vcr in the unit/integration tiers** (skillrig shells `git`, never calls the GitHub HTTP API — see Constitution Check); the e2e tier serves a *real* git remote in-process, which is not mocking the API.
