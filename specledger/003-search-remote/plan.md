...
5. **`search`** — `catalog.go` parse; convention-version gate; deterministic query matcher (`Search(catalog, query, topics)` in `skillcore` — the signature authored in data-model §5b, C10; **stdlib-only** — case-insensitive token-AND substring over `name`+`description`+`topics`, ordered by fixed-bucket score then name) + exact-string AND `--topic` filter; two-level output. *(S2, S5)*
