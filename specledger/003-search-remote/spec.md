...
- Q: Is `search` only a topic filter, and does "tag" collide with git tags? → A: **`search [QUERY]` is query-first** — the positional argument is a **query string matched against each skill's name + description**; the label/structured narrowing is a *secondary* filter, not the primary mechanism. The label concept is **renamed away from "tag"** (the metadata field becomes `metadata.x-skillrig.topics`) to remove the collision with **git tags** (which skillrig uses for immutable version pins, scheme `name-vSEMVER`). The **filter flag name is open** — `--topic` (explicit, single-dimension) vs a generic `-f/--filter` (extensible to `field=value`) — to be settled with the rest of the search surface. The matching algorithm, the searchable-index storage (is a flat catalog enough? how to keep a searchable index git-friendly?), the flag surface, and any indexing library are being settled in **Spike S5** (`research/2026-05-31-search-index-architecture.md`); new indexing/search dependencies are pre-approved.
...
1. **Given** a library whose skills declare names, versions, descriptions, and topics, **When** the maintainer regenerates the catalog, **Then** the catalog lists exactly those skills with exactly those fields — including the topics `search --tag` filters on.
