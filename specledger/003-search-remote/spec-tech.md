...
Out of scope (reserved): verification-failure exit 2 and prerequisite exit 3; `bump`; multi-client symlink materialization; `doctor`; the policy/allowlist enforcement in `policy.toml` (v1 governance); building the origin-side `skillrig index` generator (scoped by Spike S2, §8b).
...
- **S4 — Remote-git testing (was OD#7).** *Question:* what test substrate exercises the network-failure FRs? 002 used a local git **working tree** (no `file://`, no remote), which cannot simulate auth/unreachable/transient failures. *Spike covers:* `file://` (or a local bare repo) for the happy/integrity fetch path **plus** `httptest` or a fault-injection seam for the FR-017/018 (and transient-error) paths `file://` can't reproduce; whether any needed coverage is impossible without a real HTTP layer. Output: the test-seam decision + the ground-truth approach (fetched tree-SHA == raw `git` tree-SHA).
